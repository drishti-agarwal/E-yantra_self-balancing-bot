...
function jacobian_matrices = find_jacobian_matrices(eqbm_points, x1_dot, x2_dot)
  ...
  ################## ADD YOUR CODE HERE ######################
  j1=jacobian(x1_dot,[x1 x2]);
  j2=jacobian(x2_dot,[x1 x2]);
  
  for i=1:length(eqbm_points)
    f1=double(subs(j1,[x1 x2],solutions{i}));
    f2=double(subs(j2,[x1 x2],solutions{i}));
    final=[f1;f2];
    jacobian_matrices{i}=final;
   endfor
...
function [eigen_values stability] = check_eigen_values(eqbm_pts, jacobian_matrices)
  ...
  for k = 1:length(jacobian_matrices)
    ...
    for i=1:length(eigen_values{k})
     if real(eigen_values{k}(i))>0,
       flag=0;
     endif
    endfor
