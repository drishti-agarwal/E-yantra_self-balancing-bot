...
function dy = cart_pendulum_dynamics(y, m, M, L, g,  u)
  ...
  theta_dot = y(4);
  dy(1,1) = y(2);
  dy(2,1) = (((m*g/2)*sin(2*theta)) - (m*(-L)*(theta_dot^2)*sin(theta)) + u)/(M+(m*(sin(theta))^2));
  dy(3,1) = y(4);
  dy(4,1) = (((M+m)*g*sin(theta)) + (u*cos(theta)) - (m/2*(-L)*sin(2*theta)*(theta_dot^2)))/((-L)*(M+(m*(sin(theta))^2)));
...
function [A, B] = cart_pendulum_AB_matrix(m , M, L, g)
  ...
  A = [0,1,0,0;0,0,49/25,0;0,0,0,1;0,0,147/25,0];
...
function [t,y] = pole_place_cart_pendulum(m, M, L, g, y_setpoint, y0)
  ...
  eigs = [-4,-3,-1.5,-1.3];                            ## Initialise desired eigenvalues
...
function [t,y] = lqr_cart_pendulum(m, M, L, g, y_setpoint, y0)
  ...
  [A,B] =cart_pendulum_AB_matrix(m,M,g,L) ;              ## Initialize A and B matrix
  
  Q = eye(4);                   ## Initialise Q matrix
  
  R = 0.00001;                   ## Initialise R 
